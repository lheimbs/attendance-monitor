...
class UserManagerTest(TestCase):
    def test_user_manager_raises_valueerror_on_missing_email(self):
        self.assertRaises(ValueError, models.User.objects.create_user, email='', password='')

    def test_successful_user_creation(self):
        models.User.objects.create_user(email='asd<html>@test.de', password="testpassword123")
        last_user = models.User.objects.last()
        self.assertIs(last_user.check_password("testpassword123"), True)

    def test_create_successful_superuser(self):
        models.User.objects.create_superuser(email='test@test.de', password='123456password')
        last_user = models.User.objects.last()
...
class UserModelTest(TestCase):
    def test_user_model_str(self):
        user = models.User.objects.create_user(email='test@test.de', password='testpassword')
...
class WeekDayModelTests(TestCase):
    def setUp(self) -> None:
        self.weekday = models.WeekDay.objects.create(
    ...
    def test_weekday_model_str_function(self, *args):
        self.assertEqual(
            str(self.weekday),
            f"day: {models.WeekDayChoices(self.weekday.day).label}, time: {self.weekday.time}")
    ...
    def test_get_this_weeks_date_correct_day_of_the_week(self, *args):
        ...
        for day in models.WeekDayChoices.values:
            with mock.patch('django.utils.timezone.now', return_value=start_date):
                weekday = models.WeekDay.objects.create(
                    day=models.WeekDayChoices(day), time=start_date.time()
                )
...
class AccessTokenModelTests(TestCase):
    def setUp(self, *args):
        models.AccessToken.objects.create(valid_time=10)

    def test_accesstoken_model_str_function(self, *args):
        token = models.AccessToken.objects.last()
...
class CourseModelTests(TestCase):
    def setUp(self, *args):
        self.course = models.Course.objects.create(
            name="Testmodels.Course", duration=10, min_attend_time=5, ongoing=True
        )
        self.wd = models.WeekDay.objects.create(
            day=models.WeekDayChoices.MONDAY, time=TIME_FOR_TESTING
        )
    ...
    def test_is_ongoing_with_ongoing_flag_set(self, *arg):
        ...
        course = models.Course.objects.get(name="Testmodels.Course")
        course.ongoing = True
    ...
    def test_is_not_ongoing_with_ongoing_flag_not_set(self, *arg):
        ...
        self.assertIs(bool(course.is_ongoing()), False)
    ...
    def test_is_ongoing_with_current_time_in_start_time_plus_duration_interval(self, *arg):
        ...
        self.assertIs(bool(course.is_ongoing()), True)
    ...
    def test_is_not_ongoing_with_current_time_not_in_start_time_plus_duration_interval(self, *arg):
        ...
        for day in models.WeekDayChoices.values:
            ...
            weekday, _ = models.WeekDay.objects.get_or_create(
                day=models.WeekDayChoices(day), time=daytime.time()
            )
            course.start_times.add(weekday)
            self.assertIs(bool(course.is_ongoing()), False)
            course.start_times.remove(weekday)
    ...
    def test_course_get_absolute_student_register_url_without_access_token(self, *args):
        course = models.Course.objects.create(name='testcourseforurl')
        self.assertEqual(
    ...
    def test_course_get_next_date_none_for_this_week_alreay_happened(self, *args):
        self.course.start_times.clear()
        wd_2 = models.WeekDay.objects.create(
            day=models.WeekDayChoices.MONDAY,
...
class WifiInfoModelTests(TestCase):
    def setUp(self):
        self.wifi_info = models.WifiInfo.objects.create(mac="aabbccddeeff")

    def test_teacher_model_str_function(self):
        self.assertEqual(str(self.wifi_info), "aabbccddeeff")
